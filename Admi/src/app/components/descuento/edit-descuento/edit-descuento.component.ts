...
export class EditDescuentoComponent implements OnInit{
  ...
  ngOnInit(): void {
    this._route.params.subscribe(
      params=>{
        this.id = params['id'];
        console.log(this.id);
        this._adminDescuento.obtener_descuento_admin(this.id, this.token).subscribe(
          response=>{
  ...
  actualizar(actualizarForm:any){
    if (actualizarForm.valid) {
      ...
      this.load_btn=true;
      this._adminDescuento.actualizar_descuento_admin(data,this.id,this.token).subscribe(
        response=>{
          iziToast.show({
            title: 'ÉXITO',
            titleColor: '#FFD700',
